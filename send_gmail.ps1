...
# ---------------------------------------------------------------------------
function Build-EmailHtml ($fullName, $programa, $facultad, $firstActa, $rowsHtml, $sumStr, $notaComision="", $productCount=1) {
    $notaHtml = ""
    $parrafoActo = ""
    if ($notaComision) {
        ...
        $parrafoActo = "<p style='margin-top:20px;color:#2d3748;font-size:15px;text-align:justify'>Por lo anteriormente mencionado, con posterioridad se estara notificando el correspondiente acto administrativo.</p>"
    }

    # Parsear acta en dos formatos posibles:
    # Formato A: "Acta No. 2 del 04 de junio de 2026"
    # Formato B: "2- 04/06/2026"
    ...
    if ($firstActa -match "No\.\s*(\d+)\s+del\s+(\d+\s+de\s+\w+)") {
        ...
        $actaFechaText  = ", llevada a cabo durante el d&iacute;a " + $matches[2] + " de la presente anualidad"
        ...
        $actaFechaText  = ", llevada a cabo durante el d&iacute;a $day de $monthName de la presente anualidad"
    }

    # Singular o plural segun numero de productos del docente
    if ($productCount -eq 1) {
        $introSolicitud = "la solicitud presentada por usted fue considerada en el mismo y aprobada como se relaciona a continuaci&oacute;n:"
    } else {
        $introSolicitud = "las solicitudes presentadas por usted fueron consideradas en el mismo y aprobadas como se relaciona a continuaci&oacute;n:"
    }
...
if ($Test) {
    ...
    $htmlBody = Build-EmailHtml $fullName $doc.Programa $doc.Facultad $firstActa $rowsHtml $sumStr $notaComision $products.Count
...
if ($TestAll) {
    ...
    foreach ($group in $groupProducts) {
        ...
        $htmlBody = Build-EmailHtml $fullName $programa $facultad $firstActa $rowsHtml $sumStr $notaComision $products.Count
